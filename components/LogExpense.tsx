...
const LogExpense: React.FC<LogExpenseProps> = ({ onClose, onSave, onDelete, tripStartDate, currentUserId, members, initialItem }) => {
  ...
  useEffect(() => {
     if (splitMode === 'CUSTOM' && cost && splitWith.length > 0 && Object.keys(customAmounts).length === 0) {
  ...
  }, [cost, splitMode, splitWith.length]);
        ...
        try {
          ...
          reader.onloadend = async () => {
             ...
             if (items && items.length > 0) {
                 const item = items[0];
                 if (item.cost) setCost(item.cost.toString());
  ...
  const validate = () => {
      if (!title || !cost) return false;
      const total = parseFloat(cost);
      if (isNaN(total) || total <= 0) return false;

      if (splitMode === 'CUSTOM') {
          // Check if custom amounts sum up to total (approx)
          let sum = 0;
  ...
  const handleSubmit = () => {
      ...
      onSave({
          id: initialItem?.id, // Important for updates
          title,
          cost: totalCost,
          type,
  ...
  const getTypeIcon = (t: ItemType) => {
      switch(t) {
          ...
          default: return <PlusIcon className="w-5 h-5" />; // Essentials
      }
  };

  // Calculations for UI display
  const totalCostVal = parseFloat(cost) || 0;
  const equalShare = splitWith.length > 0 ? (totalCostVal / splitWith.length).toFixed(2) : '0.00';
  
  let currentCustomSum = 0;
  if (splitMode === 'CUSTOM') {
      splitWith.forEach(id => currentCustomSum += parseFloat(customAmounts[id] || '0'));
  }
  const remaining = totalCostVal - currentCustomSum;
  ...
  return (
    <div className="flex flex-col h-full bg-tactical-bg animate-fade-in relative">
        ...
        <header className="px-6 py-4 flex items-center justify-between sticky top-0 bg-tactical-bg z-20 border-b border-tactical-muted/10 w-full max-w-2xl mx-auto">
            ...
            {/* Standard Delete Icon for Non-Settlements */}
        ...
        <div className="flex-1 overflow-y-auto p-6 scrollbar-hide pb-32 w-full max-w-2xl mx-auto">
            ...
            <div className="flex flex-col items-center justify-center mb-8 mt-4">
                 ...
                 <div className="flex items-baseline justify-center gap-1">
                     <span className={`text-4xl font-bold font-display ${isSettlement ? 'text-green-500' : 'text-tactical-accent'}`}>$</span>
                     <input 
                       type="number"
                       value={cost}
                       ...
                       style={{ width: `${Math.max(1, cost.length)}ch` }}
            ...
            {/* 2. Scan Button - Only show if creating new */}
            {!initialItem && (
                <div className="mb-8">
                    <input 
                        type="file" 
                    ...
                    {isSettlement ? (
                        ...
                        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                            {[ItemType.ESSENTIALS, ItemType.FOOD, ItemType.TRANSPORT, ItemType.STAY, ItemType.ACTIVITY].map(t => (
                                ...
                                key={t}
                                onClick={() => setType(t)}
                                className={`flex items-center gap-2 px-4 py-2 rounded-full border text-xs font-bold uppercase whitespace-nowrap transition-colors ${
                                    type === t 
            ...
            {/* 4. Visibility Config - Hide for Settlements */}
            {!isSettlement && (
                <div className="mb-8">
                    ...
                    <div className="space-y-3">
                        ...
                        <div 
                            ...
                            className={`flex items-center justify-between p-4 rounded-xl border transition-all cursor-pointer ${isPrivate ? 'bg-tactical-card border-tactical-muted/50' : 'bg-transparent border-tactical-muted/20'}`}
                        >
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <div className={`font-bold uppercase text-sm ${isPrivate ? 'text-white' : 'text-gray-400'}`}>Mark as Private</div>
                                    <div className="text-[10px] text-gray-500">Visible only to you in Ledger</div>
                                </div>
                            </div>
                            <div className={`w-10 h-5 rounded-full p-0.5 transition-colors ${isPrivate ? 'bg-gray-500' : 'bg-gray-700'}`}>
                                <div className={`w-4 h-4 rounded-full bg-white shadow-sm transform transition-transform ${isPrivate ? 'translate-x-5' : 'translate-x-0'}`}></div>
                            </div>
                        </div>
                    </div>
                    <div className="h-px bg-tactical-muted/20 my-8"></div>
                         ...
                         <div className="flex items-center justify-between">
                             <div className="flex flex-col items-center gap-3">
                                 ...
                                 <div className="flex items-center gap-2 px-3 py-1.5 rounded-full border border-gray-600 bg-tactical-card">
                                     <img src={sender?.avatarUrl || `https://ui-avatars.com/api/?name=${encodeURIComponent(sender?.name || 'U')}`} className="w-5 h-5 rounded-full" />
                             ...
                             <div className="flex flex-col items-center gap-3">
                                 ...
                                 <div className="flex items-center gap-2 px-3 py-1.5 rounded-full border border-gray-600 bg-tactical-card">
                                     <img src={receiver?.avatarUrl || `https://ui-avatars.com/api/?name=${encodeURIComponent(receiver?.name || 'U')}`} className="w-5 h-5 rounded-full" />
                                            ...
                                            <div className="text-right">
                                                {splitMode === 'EQUAL' ? (
                                                    ...
                                                    <div className="flex items-center gap-1">
                                                        <span className="text-tactical-accent text-sm font-bold">$</span>
                                                        <input 
                                                            type="number"
                                                            value={customAmounts[member.id] || ''}
                                                            onClick={(e) => e.stopPropagation()}
                                                            onChange={(e) => handleCustomAmountChange(member.id, e.target.value)}
                                                            className="w-20 bg-black/30 border border-gray-700 rounded p-1 text-right text-white font-mono text-sm outline-none focus:border-tactical-accent [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none [-moz-appearance:textfield]"
                                                            placeholder="0.00"
                                                        />
                                                    </div>
                                                )}
                    ...
                    {isDeleting ? (
                        <div className="flex gap-2 animate-fade-in">
                            <button
                                ...
                                className="flex-1 bg-gray-700 hover:bg-gray-600 text-white font-display font-bold text-lg py-4 rounded-xl transition-colors uppercase"
                            >
                                Cancel
                            </button>
                            <button
                                onClick={handleDeleteClick}
                                className="flex-1 bg-red-600 hover:bg-red-500 text-white font-display font-bold text-lg py-4 rounded-xl shadow-[0_0_15px_rgba(255,0,0,0.4)] transition-colors uppercase"
                            >
                                Confirm Reverse
                            </button>
                        </div>
