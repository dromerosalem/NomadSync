...
const BudgetEngine: React.FC<BudgetEngineProps> = ({ trip, currentUserId, currentUserRole, onBack, onLogExpense, onViewLedger, onItemClick, onToggleBudgetMode, onSettleDebt }) => {
  ...
  const { 
    ...
    // Key: MemberID -> Net Amount. (Positive = Owed money by group, Negative = Owes money to group)
    ...
    // Filter for valid financial transactions
    // Recent Tx: Include Settlements to show activity
    ...
    trip.items.forEach(item => {
      ...
      // Determine who was involved for consumption logic
      const involvedIds = hasCustomSplit ? Object.keys(splitDetails) : splitWith;

      // --- CALCULATE PERSONAL SPEND & CONSUMPTION ---
      // Only for Expenses, not Settlements
      if (!isSettlement) {
          let myShare = 0;
      ...
      // --- CALCULATE DEBTS & SETTLEMENTS ---
      // Both Expenses and Settlements affect Balances.
      // Logic: Payer GIVES money/value. Consumer/Receiver TAKES money/value.
      
      // Credit the Payer (Global Net)
      // Payer gets +Cost (they paid out). 
      ...
      involvedIds.forEach(consumerId => {
          ...
          // Debit the Consumer/Receiver (Global Net)
          // Consumer gets -Share (they consumed value OR received cash settlement).
          netBalances[consumerId] = (netBalances[consumerId] || 0) - share;

          // Pairwise Logic (Direct Link)
          // If Consumer is NOT Payer, Consumer owes Payer.
    ...
    // --- SMART SPLIT ALGORITHM (SHUFFLE) ---
    // Resolve net balances into minimum transactions
    let debtors = Object.keys(netBalances)
        .filter(id => netBalances[id] < -0.01)
        .map(id => ({ id, amount: netBalances[id] }))
        .sort((a, b) => a.amount - b.amount); // Ascending (Largest debtors first: -100 before -10)

    let creditors = Object.keys(netBalances)
        .filter(id => netBalances[id] > 0.01)
        .map(id => ({ id, amount: netBalances[id] }))
        .sort((a, b) => b.amount - a.amount); // Descending (Largest creditors first: +100 before +10)
    ...
    while (i < debtors.length && j < creditors.length) {
        ...
        // The amount to settle is the minimum of the absolute debt or the credit available
        const rawAmount = Math.min(Math.abs(debtor.amount), creditor.amount);
        
        // Fix: Use the rounded amount for both the Transfer AND the internal adjustment
        // This prevents floating point drift where visual transfer != math adjustment
        ...
        // Adjust remaining balances using the CLEAN amount
        debtor.amount += amount;   // Debtor is negative, adding positive reduces debt
        creditor.amount -= amount; // Creditor is positive, subtracting positive reduces credit

        // Move pointers if settled (using small epsilon for float precision residues)
  ...
  const daysRemaining = Math.max(0, Math.ceil((new Date(trip.endDate).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24)));
      ...
      <div className="flex-1 overflow-y-auto p-6 scrollbar-hide pb-24">
         ...
         <div className="bg-tactical-card rounded-2xl p-6 border border-tactical-muted/30 shadow-lg relative overflow-hidden mb-8">
             ...
             <div className="flex justify-between items-center text-xs border-t border-white/5 pt-3">
                 <span className="text-gray-500 uppercase">Budget Reset in {daysRemaining} days</span>
